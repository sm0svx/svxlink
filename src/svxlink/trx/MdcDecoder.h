...
#ifndef MDC_DECODER_INCLUDED
#define MDC_DECODER_INCLUDED
...
/**
 * @brief   This is the base class for implementing a Mdc decoder
 * @author  Tobias Blomberg, SM0SVX & Adi Bier / DL1HRC
 * @date    2012-07-20
 */
class MdcDecoder : public sigc::trackable, public Async::AudioSink
{
  public:
    /**
     * @brief 	Create a new Mdc decoder object
     * @param 	cfg A previously initialised configuration object
     * @param 	name The name of the receiver configuration section
     * @returns Returns a new Mdc object or 0 on failure
     *
     * Use this function to create new Mdc decoder objects. What Mdc
     ...
     * contain a configuration variable Mdc_DEC_TYPE that points out the
     * decoder type to use. Valid values are: INTERNAL (only one at the moment)
     */
    static MdcDecoder *create(Async::Config &cfg, const std::string& name);

    /**
     * @brief 	Destructor
     */
    virtual ~MdcDecoder(void) {}
    ...
    */
    ...
    /**
     * @brief 	Tell the Mdc decoder to flush the previously written samples
     *
     ...
     *          detected
     * @param 	The detected Mdc message
    */
    sigc::signal<void, std::string> mdcDetected;
  ...
  protected:
     ...
     * @param 	name The name of the receiver configuration section
     *
     * MdcDecoder objects are created by calling MdcDecoder::create.
     */
    MdcDecoder(Async::Config &cfg, const std::string &name)
