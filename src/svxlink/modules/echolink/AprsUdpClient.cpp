...
int AprsUdpClient::buildSdesPacket(char *p)
{
  ...
  *ap++ = RTCP_SDES_CNAME;
  sprintf(tmp, "%s/%d", loc_cfg.mycall.c_str(), getPasswd(loc_cfg.mycall));
  addText(ap, tmp);

  *ap++ = RTCP_SDES_LOC;
  sprintf(tmp, ")EL-%.6s!%s0PHG%d%d%d%d/%06d/%03d%6s%02d%02d\r\n",
               shortCallsign().c_str(), pos,
...
string AprsUdpClient::shortCallsign()
{
  string::size_type pos = loc_cfg.mycall.find("-");
  ...
  if (pos == string::npos)
  {
    ...
    // Truncate callsign
  return loc_cfg.mycall.substr(0, pos);
  
} /* AprsUdpClient::shortCallsign */
