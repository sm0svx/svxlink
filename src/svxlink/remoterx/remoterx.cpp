/**
@file	 remoterx.cpp
@brief   Main file for the RemoteRx remote receiver for SvxLink
@author  Tobias Blomberg / SM0SVX
@date	 2006-04-14

This is the main file for the RemoteRx remote receiver for the
SvxLink server. It is used to link in remote receivers to the SvxLink
...
int main(int argc, char **argv)
{
  ...
  else
  {
    ...
    cfg_filename += "/.svxlink/remoterx.conf";
    if (!cfg.open(cfg_filename))
    {
      cfg_filename = string(home_dir);
      cfg_filename += "/.remoterxrc";
      if (!cfg.open(cfg_filename))
      {
	cfg_filename = "/etc/remoterx.conf";
	if (!cfg.open(cfg_filename))
	{
	  cerr << "*** ERROR: Could not open configuration file. Tried:\n"
      	       << "\t" << home_dir << "/.svxlink/remoterx.conf\n"
      	       << "\t" << home_dir << "/.remoterxrc\n"
	       << "\t/etc/remoterx.conf\n"
  ...
  cout << PROGRAM_NAME " v" REMOTE_RX_VERSION " (" __DATE__
      	  ") starting up...\n";
  ...
  RxHandler *rx_handler = new RxHandler(cfg);
  if (rx_handler->initialize())
  {
    app.exec();
  }
